...
async def get_london_city_briefing() -> dict:
    """One-call London ops briefing: live Tube, road congestion, street closures, and equity impact.

    Use this FIRST for broad questions like "how's London right now", "transport status",
    "any disruptions", or hackathon demos. Combines TfL live feeds with IMD-weighted impact
    scoring on the worst-affected Tube lines.
    """
    snapshot, parking = await asyncio.gather(
        fetch_london_traffic_snapshot(),
        fetch_parking_and_charging_snapshot(),
    )
    tube_issues = snapshot["tube"]["lines_not_good_service"]
    line_impacts: list[dict] = []

    for issue in tube_issues[:3]:
        line_id = normalize_line_id(issue.get("line") or "")
        if not line_id:
            continue
        try:
            # Sequential to respect TfL anonymous rate limits (~50 req/min)
            await asyncio.sleep(0.25)
            scored = await score_line_disruption_impact(line_id)
            top = (scored.get("top_impacted_wards") or [])[:3]
            line_impacts.append(
                {
                    "line": issue.get("line"),
                    "status": issue.get("status"),
                    "top_vulnerable_ward": top[0] if top else None,
                    "severity_weight": scored.get("severity_weight"),
                }
            )
        except Exception as exc:  # noqa: BLE001
            line_impacts.append({"line": issue.get("line"), "status": issue.get("status"), "error": str(exc)})

    return {
        "summary": _build_briefing_summary(snapshot, line_impacts, parking),
        "live_transport": snapshot,
        "parking_and_charging": parking,
        "equity_impact": line_impacts,
        "data_sources": ["TfL Unified API", "London IMD 2019 wards", "GLA population/GVA"],
    }


def _build_briefing_summary(
    snapshot: dict, line_impacts: list[dict], parking: dict | None = None
) -> str:
    tube = snapshot["tube"]
    roads = snapshot["roads"]
    streets = snapshot["streets"]
    parts = [
        f"Tube: {len(tube['lines_not_good_service'])} lines not on good service; "
        f"{tube['active_disruption_count']} active disruption(s).",
        f"Roads: {roads['congested_corridor_count']} congested corridors; "
        f"{roads['total_active_disruptions']} active road disruptions.",
        f"Streets: {streets['total_disrupted_segments']} disrupted segments; "
        f"{streets['closed_or_restricted_count']} closed or restricted.",
    ]
    if parking:
        ev = parking.get("ev_charging") or snapshot.get("ev_charging") or {}
        cp = parking.get("car_parks") or snapshot.get("car_parks") or {}
        if ev:
            parts.append(
                f"EV charging: {ev.get('available', 0)} available / "
                f"{ev.get('total_connectors', 0)} connectors "
                f"({ev.get('charging', 0)} charging, {ev.get('out_of_service', 0)} out of service)."
            )
        if cp:
            parts.append(
                f"Car parks: {cp.get('total_car_parks', 0)} TfL sites "
                f"({cp.get('total_listed_spaces', 0)} total spaces); "
                f"live bay occupancy {cp.get('live_occupancy_api_status', 'unknown')}."
            )
    if line_impacts:
        ward_bits = []
        for li in line_impacts:
            top = li.get("top_vulnerable_ward")
            if top:
                ward_bits.append(f"{li.get('line')}: {top.get('ward_name')} (IMD rank {top.get('imd_average_rank_rank')})")
        if ward_bits:
            parts.append("Most vulnerable wards on affected lines: " + "; ".join(ward_bits) + ".")
    return " ".join(parts)
