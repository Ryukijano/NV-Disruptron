...
1. If line unknown → `disruptron_ops__get_london_city_briefing` and read `equity_impact`.
2. For a specific line → `disruptron_ops__score_line_disruption_impact(line_id)`  
...
4. For postcode/ward context → `disruptron_ops__lookup_ward_by_postcode` or `get_ward_profile`.
5. Synthesize: **who is affected, why they are vulnerable (IMD), operational priority**.

## Full MCP mode only (DISRUPTRON_SLIM_MCP=false)
