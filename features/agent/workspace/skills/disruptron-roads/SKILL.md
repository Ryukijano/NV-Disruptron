...
1. Orient: `disruptron_ops__get_london_city_briefing` or `disruptron_ops__get_london_traffic_snapshot`.
2. **Corridor congestion** → `disruptron_ops__get_all_road_status(congested_only=true)`.
   - Report corridor names, severity (`Serious`, `Moderate`, …), active disruption count.
3. **Street works / closures** → `disruptron_ops__get_street_disruptions`.
