...
1. Broad: `disruptron_ops__get_london_city_briefing` → read `live_transport.tube`.
2. Specific line equity: `disruptron_ops__score_line_disruption_impact(line_id)`.
3. Deeper snapshot: `disruptron_ops__get_london_traffic_snapshot` if roads/EV context needed too.
